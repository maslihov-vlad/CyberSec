...
class SBT(Frame):
    ...
    def initUI(self):
        ...
        search_label = Label(self, text="Search:", background= 'orange', foreground='blue')
        ...
        select_all_button.grid(sticky=S, row=7, column=1, pady=6)

        select_failed_button = Button(self, text="Select failed options", command=self.selectFailed)
        select_failed_button.grid(sticky=S, row=7, column=2, pady=6)

        deselect_all_button = Button(self, text="Deselect all options", command=self.deselectAll)
        deselect_all_button.grid(sticky=S, row=7, column=3, pady=6)
        ...
        enforce_button.grid(sticky=S, row=6, column=2, pady=6)

        rollback_button = Button(self, text="Rollback", command=self.rollback)
        rollback_button.grid(sticky=S, row=6, column=3, pady=6)
    ...
    def saveFile(self):
        ...
        for i in self.list.curselection():
            ...
            if z == 'REGISTRY_SETTING':
                ...
                print(' reg_key:', (reg_key[i])[1:], file=f)
                print(' reg_item:', (reg_item[i])[1:], file=f)
